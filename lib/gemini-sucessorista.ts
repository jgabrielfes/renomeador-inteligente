...
import { semQualificadoresDeNome } from "./partilha/nomes";
...
export interface CasoExtraido {
  ...
  arquivos: Array<{
    ...
    /**
     * Nome de arquivo PADRONIZADO sugerido pela leitura, SEM extensão
     * ("Certidão de Óbito - João da Silva"); null quando o documento não foi
     * identificado. O cofre renomeia o anexo com ele (a extensão original
     * volta no cliente).
     */
...
7. arquivos — para CADA documento (indice 1 a ${total}): tipoDetectado (rótulo curto, ex.: "Certidão de Óbito"), documentoId = o id do catálogo abaixo em que o arquivo deve ser arquivado (null se nenhum servir) e nomeSugerido = o NOME DE ARQUIVO padronizado, SEM extensão, no padrão "<Tipo do documento> - <pessoa ou identificador>": inclua o NOME da pessoa quando o documento a identificar (ex.: "Certidão de Óbito - João da Silva", "RG - Maria de Souza", "Certidão de Casamento - João da Silva e Maria de Souza"); matrícula de imóvel com número e Registro de Imóveis (ex.: "Matrícula 12.345 - 1º RI de Guarulhos"); certidão de valor venal e IPTU com o exercício (ex.: "Certidão de Valor Venal 2025 - Rua das Flores 100"); extrato com banco e referência (ex.: "Extrato Itaú - saldo na data do óbito"); até 80 caracteres, sem barras, aspas, dois-pontos ou outros caracteres proibidos em nome de arquivo; null quando não conseguir identificar o documento. Documentos pessoais (RG/CNH/CPF/comprovante de endereço) classificam-se pelo DONO: do(a) falecido(a) → docs-falecido; do cônjuge/companheiro(a) sobrevivente → docs-sobrevivente; de herdeiro ou cônjuge de herdeiro → docs-herdeiros.
...
export async function extrairCasoDoCofre(
  ...
  const herdeirosNoObito = (Array.isArray(bruto.herdeirosNoObito) ? bruto.herdeirosNoObito : [])
    ...
    // Defesa contra qualificadores que escapem do prompt ("Pedro, maior"):
    // o que cruza com o item I é o NOME, nunca "maior/menor/idade".
    .map((n: string) => semQualificadoresDeNome(n.trim()).slice(0, 120))
    ...
    arquivos: arquivosSaida
      .map((a) => {
        ...
        return {
          ...
          // Nome de arquivo: sem caracteres proibidos, sem extensão colada.
          nomeSugerido: (() => {
            const bruto = limpar(item.nomeSugerido, 80);
            if (!bruto) return null;
            const semExt = bruto.replace(/\.(pdf|jpe?g|png|webp|docx|xlsx|txt)$/i, "");
            const limpo = semExt.replace(/[\\/:*?"<>|]+/g, " ").replace(/\s+/g, " ").trim();
            return limpo || null;
          })(),
        };
      })
