/**
 * Municípios do Brasil — base ESTÁTICA para o autocomplete de comarcas do
 * módulo de correspondentes (camada 4, pilar B).
 *
 * Fonte: pacote `brazilian-cities` (dados do IBGE), gerado uma única vez —
 * o código é o do IBGE SEM o dígito verificador (6 dígitos; os dois
 * primeiros são a UF). 5.587 entradas. Hook futuro: substituir/atualizar
 * pela API de localidades do IBGE ou pela base de serventias do CNJ/ONR.
 *
 * A base fica NO SERVIDOR (importada por server actions) — o cliente
 * pergunta por `buscarMunicipios`, nunca carrega a lista no bundle.
 */
...
/** [ibge, nome, uf] — tuplas para o arquivo ficar compacto. */
type Tupla = [number, string, string];
...
export function municipioPorIbge(ibge: number): Municipio | null {
  const t = TUPLAS.find((x) => x[0] === ibge);
...
export function buscarMunicipios(consulta: string, limite = 12): Municipio[] {
  ...
  for (const [ibge, nome, uf] of TUPLAS) {
    const n = semAcento(nome);
    if (n.startsWith(q)) comeca.push({ ibge, nome, uf });
    else if (n.includes(q)) contem.push({ ibge, nome, uf });
...
const TUPLAS: Tupla[] = [
  ...
  [539901, "Águas Claras", "DF"],
  [530010, "Brasília", "DF"],
  [530020, "Brazlândia", "DF"],
  [530030, "Candangolândia", "DF"],
  [530040, "Ceilândia", "DF"],
  [530050, "Cruzeiro", "DF"],
  [530060, "Gama", "DF"],
  [530070, "Guará", "DF"],
  [539928, "Itapoã", "DF"],
  [539930, "Jardim Botânico", "DF"],
  [530080, "Lago Norte", "DF"],
  [530090, "Lago Sul", "DF"],
  [530100, "Núcleo Bandeirante", "DF"],
  [530110, "Paranoá", "DF"],
  [539913, "Park Way", "DF"],
  ...
  [530160, "São Sebastião", "DF"],
  [539929, "SCIA", "DF"],
  [539933, "SIA", "DF"],
  [530170, "Sobradinho I", "DF"],
  ...
  [412330, "Santa Cruz Monte Castelo", "PR"],
  [412340, "Santa Fé", "PR"],
