...
- **Custas sobre o MAIOR valor**: o bem carrega `valorVenal`/`valorAvaliacao` (opcionais, lançados no acervo); a base de escritura e de registro é o maior entre atribuído, venal e avaliação (`baseDeCustaMaior` em `acervo-view.tsx`, e o `max` no memo `custos` do client) — Enunciado 7 do CNB/SP.
