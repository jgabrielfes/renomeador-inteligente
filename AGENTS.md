...
- **Busca textual: no BANCO, com o termo na query string** (`?busca=`), lida por `parseBusca` de `lib/admin.ts`. O componente é o `SearchFilter` (`components/admin/search-filter.tsx`): **sem botão e sem Enter** — a digitação vira navegação por debounce (400 ms), voltando à página 1, com o input controlado localmente para não piscar enquanto o servidor re-renderiza. Referência: `/admin/usuarios?busca=` (nome OU e-mail, `mode: "insensitive"`).
